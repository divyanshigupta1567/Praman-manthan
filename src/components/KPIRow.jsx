...
function KpiCard({ label, value, index }) {
  ...
  return (
    <div className="kpi-card" data-spotlight data-reveal data-reveal-delay={index % 4}>
    ...
    <section className="kpi-row" aria-label="Key metrics">
      ...
      >
        <div className="kpi-grid">
          {KPI_DEFS.map(({ key, label }, i) => (
            <KpiCard key={key} label={label} value={metrics?.[key] ?? 0} index={i} />
          ))}
        </div>
      </SectionState>
    </section>
